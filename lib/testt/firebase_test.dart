  ...
  await ref.get().then((QuerySnapshot<Object?> snapshot) async {
    for (final docSnapShot in snapshot.docs) {
      ...
      waypoints.forEach((element) async {
        ...
        DocumentReference<Map<String, dynamic>> docRef = element as DocumentReference<Map<String, dynamic>>;
        docRef.get().then((DocumentSnapshot<Map<String, dynamic>> value) {
          ...
          for (var value in pictures) {
            DocumentReference<Map<String, dynamic>> pictureRef = value as DocumentReference<Map<String, dynamic>>;
            pictureRef.get().then((DocumentSnapshot<Map<String, dynamic>> value) {
...
Future<void> testFirebase2() async {
  ...
  games = await FirebaseFirestore.instance.collection("games").get().then((QuerySnapshot<Object?> snapshot) async {
...
class FirebaseTestView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    //ProviderScope.containerOf(context).read(currentGameViewModelProvider.notifier).initGame(Difficulty.easy, context);
    return Scaffold(body: Consumer(
      builder: (context, ref, _) {
        WidgetsBinding.instance.addPostFrameCallback((timestamp) {
          ref.read(currentGameViewModelProvider.notifier).initGame(Difficulty.easy, context);
        });
            ...
            data: (state) {
              return ListView.builder(
                  itemBuilder: (context, index) {
                    return ListTile(
                      title: Text(state.currentGame?.name.toString() ?? ""),
                    );
                  },
                  itemCount: 2);
            });
